...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const eventType = body.event_type || 'order'
    ...
    if (body.payment_screenshot_path) {
      const { data } = await adminClient.storage
        ...
        .createSignedUrl(body.payment_screenshot_path, 60 * 60)
      if (data?.signedUrl) {
    ...
    for (const dest of destinations) {
      try {
        let resp
        if (photoUrl && !isTest) {
          resp = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendPhoto`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: dest.chat_id, photo: photoUrl, caption: message, parse_mode: 'Markdown' }),
          })
        } else {
          resp = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: dest.chat_id, text: message, parse_mode: 'Markdown' }),
          })
        }
